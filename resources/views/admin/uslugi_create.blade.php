...
@section('title', 'Dodaj usługę - Admin')
...
<div class="admin-container">
    ...
    <div class="doc-card form-card" style="max-width:600px;margin:30px auto; padding: 25px;">
        <h2 class="card-title">Dodaj nową usługę dla dr {{ $profile->user->first_name }} {{ $profile->user->last_name }}</h2>

        <form action="{{ url('/admin/uslugi') }}" method="POST">
            @csrf
            <input type="hidden" name="doctor_id" value="{{ $profile->id }}">
